...
void writeToIndelObject(indel_t* mom_indel, bcf1_t *b, bcf_hdr_t* h, int* g, int d, int mq, int& flag, int i, int i0) 
{
  ...
  /*allocate memory for lik objects*/
  mom_indel->lk = (int*) malloc(sizeof(int) * b->gi[i0].len);
  mom_indel->n_alleles = b->n_alleles;
  mom_indel->gt_len = b->gi[i0].len;
  for (int j = 0; j < b->gi[i0].len; ++j) // R/R, R/A and A/A
    mom_indel->lk[j] = likl[j];
  if (mom_indel->rms_mapQ < MIN_MAPQ || mom_indel->depth < MIN_READ_DEPTH) 
    flag =1;
  //printf("\nindel:alt %s, length: %d\n", b->alt, b->gi[i0].len);
